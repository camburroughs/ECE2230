...
// This test driver is made to test boundary conditions, insertions in a full list,
// duplicate keys, and deletion of keys that are not present. This version will use 
// a small, even table size to show the effect on bad table sizes for the double
// probing type.
void UD1(void)
{
    table_t *table;
    int i, result, *dp, *tmp;
    int size  = 8; //small even table
    char *keys[] = { "this", "is", "my", "test", "driver", "welcome", "and", "thank", "you" };
    char *extra = "ghost";
    data_t deleted;

    table = table_construct(size, ProbeDec);
    assert(table != NULL);

    //test deletion of empty table
    assert(table_delete(table, strdup(keys[0])) == NULL);
    printf("\nEmpty list, key \"%s\" could not be deleted\n\n", keys[0]);

    //build full table
    for(i = 0; i < size-1; i++) {
	dp =(int*)malloc(sizeof(int));
	*dp = i*22;
	table_insert(table,strdup(keys[i]),dp);
	dp = NULL;
    }
    table_debug_print(table);

    //try to add more items (should pass without assertion error)
    dp = (int*)malloc(sizeof(int));
    *dp = i*22;
    result = table_insert(table, strdup(keys[i]), dp);
    dp = NULL;
    if(result == -1)
	printf("Table full, \"%s\" could not be inserted\n\n", keys[i]); 
    table_debug_print(table);
    ...
    *dp = 123;
    ...
    assert(table_insert(table, strdup(keys[0]), dp) == 1);
    tmp = table_retrieve(table, keys[0]);
    assert(*tmp == 123);
    printf("Key \"%s\" already in list, pointer updated to %d\n\n", keys[0], *dp);

    //test deletion of key not in list
    assert(table_delete(table, extra) == NULL);
    printf("Could not delete \"%s\" because it is not in the table\n", extra);
   
}

// This test driver is made to test boundary conditions, insertions in a full list,
// duplicate keys, and deletion of keys that are not present. This version will use 
// a small, prime table size to show the effect a good, prime table size has for the
// double probing type. It will also demonstrate the difference between "full" tables
// between chain type and open addressing type tables
void UD2(void)
{
    table_t *table;
    int i, result, *dp, *tmp;
    int size  = 5; // small prime table
    char *keys[] = { "apple", "orange", "kiwi", "banana", "grape", "lime", "lemon", "lime", "cherry"};
    char *extra = "ghost";
    ...
    *dp = 123;
    ...
    //test rehash to size 7
    table = table_rehash(table, 7);
    table_debug_print(table);
}
...
void getCommandLine(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "m:a:h:i:t:s:erbvu:")) != -1)
        switch(c) {
