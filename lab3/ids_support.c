 ...
 * MP3
 *
 * Propose: 	MP3 extends the purpose of MP2 by adding a public llist.c 
 *		sorting function to access private llist.c sorting algorithms.
 *		The menu now has new functionality to handle sorting the 
 *		previously unsorted queue by either destination ip address or
 *		generator id using one of four different sorting algorithms.
 *		The time library is used to measure the effifiency of the 
 *		different sorting algorithms on different size lists.
 *
 * Assumptions: User will be familiar with how to use the program, as all menu 
 *		and validation prints will be surpressed except for one final 
 *		time summary print.
 *
 *		All surpressed output comments will be marked with TODO to make 
 *		easier to identify in vim.
 *
 * 		ids_ functions will not be used in llist.c
 *
 *		User will only use Queue and List commands on their respective 
 *		linked lists.
 *
 * Bugs:
 *
...
/* ids_sort_genid is used to sort an unsorted list by generator id. 
 *
 * This function contains the only unsurpressed print statement. 
 *
 * For the alert data we want to sort from lowest ID up, so
 * closer to the front means a smaller ID.
 *
 * The function expects a valid sort_type and will output the final
 * elapsed sorting time at the end of the function.
 */
void ids_sort_genid(llist_t *list_ptr, int sort_type)
{
    clock_t start, end;
    ...
    start = clock();
    llist_sort(list_ptr, sort_type, ids_compare_genid); // sort list
    end = clock();
    elapse_time = 1000.0 * ((double) (end - start)) / CLOCKS_PER_SEC;
    
    //verify initial and final list entry count are the same.
    assert(llist_entries(list_ptr) == initialcount);    
    printf("%d\t%f\t%d\n", initialcount, elapse_time, sort_type);
}


/* ids_sort_destip is used to sort an unsorted list by destination ip
 * address. 
 *
 * This function contains the only unsurpressed print statement. 
 *
 * For the alert data we want to sort from highest IP addr down, so
 * closer to the front means a larger destination IP address.
 *
 * The function expects a valid sort_type and will output the final
 * elapsed sorting time at the end of the function.
 */
void ids_sort_destip(llist_t *list_ptr, int sort_type)
{
    clock_t start, end;
    double elapse_time;    /* time in milliseconds */
    ...
    start = clock();
    llist_sort(list_ptr, sort_type, ids_match_destip); // sort list
    end = clock();
    elapse_time = 1000.0 * ((double) (end - start)) / CLOCKS_PER_SEC;
...
/* ids_append_rear is similar to ids_add_rear and is used with
 * unsorted Queues.
 * 
 * Unlike ids_add_rear, this function does not remove copies added
 * to the list.
 * 
 * The function expects a valid list ptr, genid, and destip and will 
 * create a new list entry with all other entry members set to zero.
 *
 * Final code will have output commented out.
 */
...
//TODO    printf("Appended %d onto queue\n", new_ptr->dest_ip_addr);
